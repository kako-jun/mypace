...
- **teaserタグ**: mypace独自仕様だが、Nostrのタグ構造に従う
- 他のクライアントはteaserタグを無視するだけで害はない
...
Kind 1として投稿するが、contentを分割してteaserタグを使用:
...
| 投稿の種類 | teaserタグ | 処理 |
|------------|----------|------|
| mypace長文投稿 | あり | content末尾のリンク除去 + teaserタグ内容を結合して全文表示 |
...
```typescript
if (hasTeaserTag(event)) {
  const foldContent = getTeaserContent(event.tags)
...
- [ ] teaserタグの検出関数追加 (`hasTeaserTag`, `getTeaserContent`)
- [ ] タイムライン: teaserタグ有無で表示分岐
- [ ] 個別ページ: teaserタグ結合処理
- [ ] GUI側の切り詰め閾値を420→280に変更

### Phase 2: 投稿対応

- [ ] 280文字超で自動分割 + teaserタグ生成
...
- teaserタグはmypace独自仕様（他クライアントは無視）
- READ MOREリンクのドメインはデプロイ先に合わせる
- 将来的にteaserタグが他のクライアントでも採用される可能性を考慮
