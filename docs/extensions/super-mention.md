...
エディタ上部の `@@` ボタンをクリックするか、エディタ内で `@@` と入力すると、モーダルポップアップが表示される。
...
- モーダル形式のポップアップ（背景クリックで閉じる）
- 検索は部分一致（「無限城」で「鬼滅の刃 無限城編」がヒット）
- 複数キーワード対応（スペースまたはカンマ区切りでAND検索）
  - 例: 「鬼滅 無限」→「鬼滅」AND「無限」を両方含む候補のみ表示
- 矢印キーで候補選択、Enter/Tabで決定、Escで閉じる
- 空の検索では最近の履歴を表示
- 履歴とWikidataから同時検索
