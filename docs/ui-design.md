...
- **アクティブ判定**: mypace only ON、言語フィルタ設定、NGワード設定のいずれかがあればアクティブ

### FilterPanel
「Filter」ボタンをクリックすると、すぐ下にポップアップでフィルタUIが表示される:
- **OK word**: 投稿内容に含まれるキーワードで検索（虫眼鏡アイコン）
- **NG word**: 指定キーワードを含む投稿を非表示（禁止アイコン、赤色）
  - カンマ区切りで複数指定可能
  - 大文字小文字を区別しない
- **mypace only**: #mypace投稿のみ表示（トグル）
- **言語フィルタ**: ドロップダウンで言語選択
- **タグフィルタ**: 選択中のタグを表示（AND/OR切り替え、個別削除可能）

### フィルタ操作
- **保存ボタン**: 右下の「Save」で設定を保存し、検索/タイムラインに遷移
  - 変更がある場合は赤いドットインジケータを表示
- **解除ボタン**: 「Clear」でデフォルト設定にリセット（mypace only=ON, 言語=なし, OK word=空, NG word=空）
