...
- **イエローステラ**: 常に取り消し可能
- **カラーステラ**: 取り消し不可（支払い済みのため）

イエローを取り消すと、イエローのみ0になり、カラーステラは残る。
イエローのみで他の色がない場合は、リアクション自体が削除される。
...
- **イエロー**: 常に取り消し可能（カラーステラがあっても）
- **カラーステラ**: 取り消し不可（支払い済み、残る）
...
- 各色の星の下に、その色の数字（枠付きテキスト、リプライ数等と同じスタイル・サイズ）
- ステラが0個の場合も「0」を表示（リプライ・リポストと同様）
- 星部分をクリック → カラーピッカーが表示
- 自分の投稿でも見た目・クリック動作は同じ（カラーピッカー内のボタンが無効になるだけ）

### カラーピッカー

5色の星ボタンが横に並び、各星の下に内訳表示用のアイコンがある:
...
- 各星の下の⋮アイコンをクリック → その色のリアクター一覧が表示

### リアクター一覧（色別）

カラーピッカー内の⋮アイコンをクリックすると、その色のリアクター一覧がポップアップ表示される:
...
- 色ごとに別々のポップアップ
- 自分のイエローステラのみ「Remove」ボタンで取り消し可能
