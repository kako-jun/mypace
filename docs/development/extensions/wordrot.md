...
| 用語                   | 英語    | 説明                                                                                  |
| ---------------------- | ------- | ------------------------------------------------------------------------------------- |
| ワードロット (Wordrot) | Wordrot | 収集・合成した単語。黄色背景（#F1C40F）の16bitピクセルアート画像付き。                |
...
- 全ワードロット: 黄色背景（#F1C40F）の16bitピクセルアート — 収集も合成も同じ見た目
...
### 画像生成（FLUX.1-Schnell on Workers AI）

- **NSFW誤判定が頻発する**: Cloudflare Workers AI の FLUX.1-Schnell は安全フィルターが過敏。「cute」「squishy」「baby」「pet」「buddy」等の単語を含むプロンプトでNSFW判定されることがある。synthesis専用テンプレート（可愛い生物を描く指示）は全滅したため廃止した
- **抽象語は黄色一色になる**: 「ポイント」「タイム」等の抽象語はFLUX.1が何も描けず背景色だけの画像（<2KB）を生成する。describeWordForImageプロンプトで具体物へのマッピングを強化（「point → a bright red target bullseye」等）。さらに2KB未満チェックで失敗扱いにしてリトライする仕組みを追加
- **ドット絵にならない**: 「16-bit pixel art」だけでは滑らかなイラストになる。「32x32 pixel sprite scaled up with clearly visible large square pixels」「Chunky blocky pixels」「No anti-aliasing, no smooth gradients」等の明示指定が必要。それでもFLUX.1はSNES級の粗いドット絵は苦手
- **8ステップでは品質が低い**: steps=8 は高速だがプロンプト追従度が低い。改善にはsteps増加が有効だがコスト増

### DB管理

- **完全リセット**: `POST /api/wordrot/reset-all` で全5テーブル削除 + nostr.build画像削除。wranglerでの手動DELETE文は消し漏れが起きるので使わない
- **画像全リトライ**: `POST /api/wordrot/retry-all-images?force=true` で全画像再生成（doneのものも含む）
- **画像のみリセット**: `POST /api/wordrot/clear-all-images` で画像のみ削除しデータは残す
- **DB列 `image_url_synthesis`, `image_hash_synthesis`, `image_status_synthesis`**: 2026-03-08以降は未使用。DB列は残存するが書き込み・参照はしない

### 名詞抽出

- **英語名詞の判定はLLMに一任**: regex抽出はカタカナのみ対象。英語はregexだと動詞・形容詞も拾ってしまうため廃止
- **カタカナregexの先頭ー禁止**: 「ーホビー」のように先頭が長音記号のものはマッチしない（`/[\u30A1-\u30F6][\u30A1-\u30F6\u30FC\u30FD\u30FE]+/g`）
- **抽出結果は `wordrot_event_words` にキャッシュされる**: reset-allで消すか、投稿ごとに再抽出される

### 合成

- **Workers AI (Qwen3-30B) が不安定**: タイムアウトや無応答が起きる。フロント側に30秒タイムアウトを追加済み
- **合成レシピはキャッシュされる**: `wordrot_syntheses` テーブルの `UNIQUE(word_a_id, word_b_id, word_c_id)` で同じ組み合わせはLLMを呼ばない

### UIの設計判断

- **リトライは長押し隠し機能**: 画像の上にリトライアイコンが重なると見た目が悪いため、500ms長押しで発動する隠し機能にした
- **インベントリは1セクション「Wordrot」**: harvest/synthesisの分離を撤廃。同じword_idの重複はフロントで排除
