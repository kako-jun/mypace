...
- **ステラ**: Kind 7 で `stella` タグがある。e タグが複数ある場合は `root` マーカー付き → `reply` マーカー付き → 最後の e タグの優先順で対象投稿を決定
- **リプライ**: Kind 1 で `e` タグがある（返信先がある）。p タグに含まれるユーザーに通知（投稿者自身は除外）。e タグは NIP-10 に従い `reply` マーカー → `root` → 末尾の優先順で対象投稿を決定。重複チェックは `source_event_id` + `actor_pubkey` の組み合わせで判定
- **リポスト**: Kind 6。最後の e タグを対象投稿として使用
...
MY PACE を開いていなくても気づけるよう、Web Push通知を実装済み。

- 設定画面でオプトイン（デフォルトはオフ）
- 「All notifications」「Replies only」の粒度選択
- プッシュ通知タップで該当投稿ページに遷移
- VAPID認証によるセキュアな配信（`services/web-push.ts`）
- 無効な購読は自動クリーンアップ
