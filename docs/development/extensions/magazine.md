...
操作ボタン（↑↓×）は投稿カードの外側（上部右寄せ）に配置される。
ピン留めボタンと同じ共通スタイル（`post-card-top-controls`）を使用。
...
| コンポーネント | 場所 | 役割 |
|--------------|------|------|
| `MagazineSection` | `components/magazine/` | ユーザーページ内マガジン一覧 |
...
| `TimelinePostCard` | `components/timeline/` | 投稿カード（`topControls`で操作ボタン受け取り） |
