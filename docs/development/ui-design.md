...
```
[アバター] 投稿者名
           タイムスタンプ · 📊 詳細 / インプレ · Blog/NPC
```

- **アバター**: クリックでユーザーページへ遷移
- **投稿者名**: 白文字・黒縁（テーマに関係なく読める）
- **タイムスタンプ**: 相対時間（例: `2h ago`）または日付（例: `2025-01-13`）
- **閲覧数**（#mypace投稿のみ）: ` · ` 区切りで `[BarChart2アイコン] 詳細 / インプレ` 形式
  - 0/0でも表示
  - 詳細: 投稿詳細を開いたユニークユーザー数
  - インプレ: タイムラインに表示されたユニークユーザー数
- **ラベル**: Blog（kind 30023）、NPC（kind 42000）の場合に ` · ` 区切りで表示
...
- 文字数制限: 4200文字
- **上段（row-1）**:
  - アバター: クリックでプロフィールページへ
  - 📄ボタン: テキストファイルインポート（本文が空の時のみ表示）
  - @@ボタン: スーパーメンション（本文が空の時のみ表示）
  - LONGボタン: 長文モードへ切り替え（右寄せ）
- **中段（row-2）**:
  - 🖼ボタン: 画像ピッカー（画像添付/シール追加を統合）
  - 🎨ボタン: 手描き入力
  - 🎤ボタン: 音声入力
  - 📍ボタン: 位置情報追加
- **テキストエリア**:
...
- **画像ピッカー詳細**（sticker.md参照）:
  - 「Select Image」モーダルで画像選択
  - アップロード、URL入力、履歴から選択可能
  - 選択後に「Embed」（本文挿入）か「Sticker」（シール追加）を選択
...
### リポスト（NIP-18 kind 6）
- リポストボタンクリックで確認ポップアップ表示
  - 「Repost?」タイトル、Yes/Noボタン
  - ボタン右側に吹き出し形式で表示（ReactorsPopupと同じパターン）
  - ポータルベースで画面上にオーバーレイ表示
  - 外側クリックまたは×ボタンでキャンセル
- Yesで再投稿実行
...
```
['aurora', '#0a1628', '#1a3a5c', '#1a3a5c', '#0a1628']
```
...
- **URL**: 緑色リンク、新しいタブで開く
- **画像URL**: インライン画像表示
  - 拡張子ベース: `.jpg`, `.png`, `.gif`, `.webp`, `.svg` で終わるURL
  - キーワードベース: パスに `image`, `img`, `photo`, `picture`, `thumbnail`, `thumb` を含む拡張子なしURL（例: `/api/article-image?id=5`）
  - キーワードベースの画像は読み込み失敗時に通常リンクへフォールバック（拡張子ベースは404表示）
...
メディアクリック時にLightBoxモーダルで表示:
- 背景: 半透明黒（`rgba(0,0,0,0.9)`）
- ESCキーまたは背景クリックで閉じる
- 右上に×ボタン
- メディアクリック領域はメディアのみ（親要素への伝播を防止）
- 表示中はbodyスクロールを無効化
...
|--------|--------|------|
| 画像 | その他 | 拡大表示 |
| 動画 | .mp4, .webm, .mov, .avi, .mkv | コントロール付き再生 |
| 音声 | .mp3, .wav, .ogg, .m4a, .aac, .flac | オーディオプレイヤー |
...
- 画像URLは埋め込み対象外（インライン画像として別処理）
  - 拡張子ベース（`.jpg`等）とキーワードベース（パスに`image`等を含む）の両方が対象
...
7. **その他の処理** - 画像URL（拡張子ベース → キーワードベースの順）、リンク、ハッシュタグ、カスタム絵文字
...
- **保存キー**: `mypace` （ルートキー内の `draft` プロパティに格納）
