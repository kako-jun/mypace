...
## 503エラー対策: ハイブリッドキャッシュ戦略

### 背景（課題）

Cloudflare Workersでリレー（WebSocket）接続を行うと、リソース制限により503エラー（Error 1102: Worker exceeded resource limits）が発生する。

- 原因: SimplePoolによるWebSocket接続がWorkerのCPU/メモリを圧迫
- 再現: 連続10回程度のリクエストで発生
- 特徴: 一度発生すると連続して発生し、約10秒後に回復

### 解決策: タイムラインのみキャッシュ専用

**ルール:**
- `GET /api/timeline` → **常にキャッシュのみ**（リレー接続しない）
- 他のエンドポイント → **通常通りリレー使用**

**理由:**
- タイムラインは50件を一括取得するため、リレー接続の負荷が最も高い
- enrichなど他のエンドポイントは、表示された投稿の分だけ取得するため負荷が限定的
- キャッシュにない投稿のenrichは取得しないため、無駄なリレー接続が発生しない

### 設定値

| 環境変数 | 値 | 説明 |
|---------|-----|------|
| `RELAY_COUNT` | `2` | リレー接続数（timeline以外で使用） |
| `FETCH_MULTIPLIER` | `1` | 取得倍数 |
| `CACHE_TTL_MS` | 7日 | キャッシュ有効期限 |
| `CACHE_CLEANUP_AGE_MS` | 30日 | キャッシュ物理削除期限 |

### 動作フロー

```
タイムライン表示:
  1. GET /api/timeline → キャッシュから50件取得（リレー接続なし）
  2. POST /api/events/enrich → 50件のメタデータをリレーから取得
     - reactions, replies, reposts: リレーから
     - profiles: キャッシュ優先、なければリレーから
     - views: D1から

個別ページ表示:
  1. POST /api/events/by-ids → キャッシュ優先、なければリレーから
  2. POST /api/events/enrich → 同上

投稿:
  1. POST /api/publish → 全リレーに送信 + キャッシュに保存
```

### キャッシュの更新方法

現状はpublish時のみキャッシュが更新される。他ユーザーの投稿をキャッシュに取り込むには、将来的に以下のいずれかが必要:

1. **Cron/Scheduled Worker**: 定期的にリレーから取得してキャッシュ更新
2. **手動更新**: 管理者が定期的にキャッシュを更新

### 制約

- タイムラインに表示されるのはキャッシュ済みの投稿のみ
- 他ユーザーの新規投稿はキャッシュに入るまで表示されない
- 「過去分を取得」ボタンはキャッシュ内のデータのみ取得可能
