...
| 機能 | 状態 | 備考 |
|------|------|------|
| イエローステラ | ✅ | 無料で無制限に付けられる |
| ステラカウント表示 | ✅ | 投稿カード、ユーザーページ |
| カラーステラ表示 | ✅ | 5色対応 |
| Lightning支払い | ✅ | カラーステラ付与時に自動支払い |
| インベントリ | ✅ | ウォレット残高→ステラ自動変換 |
| 両替機能 | ✅ | satsを最大金種で表示 |
| ユーザー累計ステラ（もらった数） | ✅ | D1に保存、ユーザーページに表示 |
| ホーム右下ウィジェット | ✅ | MyStatsWidget |

### 未実装（実装予定）

| 機能 | 優先度 | 備考 |
|------|--------|------|
| あげた数の集計 | 高 | D1に記録、GIVER文化の基盤 |
| ペイウォール | 高 | ステラがないと折りたたみ解除不可 |

#### あげた数の集計 - 実装メモ

**DB実装**

新しいテーブルは不要。既存の `user_stella` テーブルにインデックスを追加するだけで対応可能。

```sql
-- 現在あるインデックス
idx_user_stella_author ON user_stella(author_pubkey)  -- もらった数用

-- 追加すべきインデックス
CREATE INDEX IF NOT EXISTS idx_user_stella_reactor ON user_stella(reactor_pubkey);
```

- `author_pubkey` で集計 → もらった数（実装済み）
- `reactor_pubkey` で集計 → あげた数（インデックス追加のみ）

データは同じ行に両方の情報（誰が誰に）が入っているため、二重管理にならない。

**UI表示**

表示場所:
- 右下のスタッツ（MyStatsWidget）
- ユーザーページのスタッツ

デザイン要件:
- 「もらった数」と「あげた数」は同じ面積で表示
- どちらがもらった/あげたか、はっきり分かるデザインにする
- 右下のスタッツは面積が惜しいので、数秒ごとに切り替え表示でもOK

### 未実装（優先度低）

| 機能 | 備考 |
|------|------|
| GIVER/TAKER比率表示 | あげた数の集計が前提 |
| ユーザータイプ判定 | SUPER GIVER等のラベル |
| ランキング | GIVERランキング等 |
| 詳細統計 | 最も付けた相手、連続付与日数等 |
