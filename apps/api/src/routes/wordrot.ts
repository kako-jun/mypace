...
// LLM prompts
const EXTRACT_NOUNS_PROMPT = `あなたは日本語テキストから名詞を抽出する専門家です。

以下のテキストから、収集対象となる「名詞」を抽出してください。

【対象とする名詞】
- 具体的な物（りんご、車、本、コーヒー）
- キャラクター名（マリオ、ピカチュウ、ドラえもん）
- 人名・有名人名（田中、イーロン・マスク）
- 概念・抽象名詞（愛、勇気、魔法、自由）
- 場所・地名（東京、城、森、アメリカ）
- 動物・植物（猫、犬、桜、バラ）
...
wordrot.post('/collect', async (c) => {
  ...
  if (!wordRecord) {
    ...
    const nsec = c.env.NOSTR_BUILD_NSEC
    if (nsec) {
      generateWordImage(ai, db, nsec, wordRecord.id, word).catch(console.error)
    } else {
      console.error('[collect] NOSTR_BUILD_NSEC not configured')
    }
  } else {
