...
views.post('/batch-record', async (c) => {
  const { eventIds, viewType, viewerPubkey } = await c.req.json<{
    eventIds: string[]
  ...
  const batch = limitedIds.map((eventId) => stmt.bind(eventId, viewerPubkey, viewType, now))

  await c.env.DB.batch(batch)

  return c.json({
    success: true,
    recorded: limitedIds.length,
  })
})
...
views.post('/:eventId', async (c) => {
  ...
  `
    ...
    .bind(eventId, viewerPubkey, viewType, now)
    .run()
