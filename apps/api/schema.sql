...
-- =====================================================
-- Word Alchemy (ワード錬金術) tables
-- =====================================================

-- Words master table (global word dictionary)
CREATE TABLE IF NOT EXISTS alchemy_words (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  text TEXT NOT NULL UNIQUE,              -- Word text (e.g., "マリオ", "ファイア")
  image_url TEXT,                         -- nostr.build image URL (generated by SD-XL)
...
CREATE INDEX IF NOT EXISTS idx_alchemy_words_text ON alchemy_words(text);
CREATE INDEX IF NOT EXISTS idx_alchemy_words_discovered_by ON alchemy_words(discovered_by);
CREATE INDEX IF NOT EXISTS idx_alchemy_words_discovery_count ON alchemy_words(discovery_count DESC);

-- User word collection (user's inventory)
CREATE TABLE IF NOT EXISTS alchemy_user_words (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  pubkey TEXT NOT NULL,
  ...
  FOREIGN KEY (word_id) REFERENCES alchemy_words(id),
  UNIQUE(pubkey, word_id)
);

CREATE INDEX IF NOT EXISTS idx_alchemy_user_words_pubkey ON alchemy_user_words(pubkey);
CREATE INDEX IF NOT EXISTS idx_alchemy_user_words_word ON alchemy_user_words(word_id);

-- Synthesis history (A - B + C = Result)
CREATE TABLE IF NOT EXISTS alchemy_syntheses (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  FOREIGN KEY (word_a_id) REFERENCES alchemy_words(id),
  FOREIGN KEY (word_b_id) REFERENCES alchemy_words(id),
  FOREIGN KEY (word_c_id) REFERENCES alchemy_words(id),
  FOREIGN KEY (result_word_id) REFERENCES alchemy_words(id),
  UNIQUE(word_a_id, word_b_id, word_c_id)
);

CREATE INDEX IF NOT EXISTS idx_alchemy_syntheses_result ON alchemy_syntheses(result_word_id);

-- Event word cache (cached noun extraction results)
CREATE TABLE IF NOT EXISTS alchemy_event_words (
...
CREATE TABLE IF NOT EXISTS alchemy_image_queue (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE INDEX IF NOT EXISTS idx_alchemy_image_queue_status ON alchemy_image_queue(status);
