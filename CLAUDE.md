...
- 投稿・閲覧（mypaceタグ付き投稿のみ表示、オフで全投稿表示）
...
- NIP-07対応（ブラウザ拡張）
- 鍵のエクスポート・インポート
- フィルタ機能（ヘッダーのフィルタボタンからポップアップ）
  - mypaceトグル（サーバーサイドフィルタ）
  - OKキーワード・OKタグ（本文＋ユーザー名を検索）
  - NGキーワード・NGタグ（指定を含む投稿を非表示）
  - 言語フィルタ
  - URL形式: `/?q=...&tags=...&ngtags=...&ng=...&mypace=off&lang=...`
  - 入力はスペースまたはカンマ区切り
